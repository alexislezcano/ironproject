import Link from 'next/link'
import Image from 'next/image'
import { getWhatsAppURL } from '@/lib/whatsapp'

const WA_PATH =
  'M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z'

export default function Footer() {
  const waURL = getWhatsAppURL({ type: 'general' })

  return (
    <footer
      style={{
        backgroundColor: 'var(--ip-bg)',
        borderTop:       '1px solid var(--ip-border)',
      }}
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">

        {/* Main footer grid */}
        <div
          className="grid grid-cols-2 md:grid-cols-4 gap-10 py-14"
          style={{ borderBottom: '1px solid var(--ip-border)' }}
        >

          {/* Brand */}
          <div className="col-span-2 md:col-span-1">
            <Link href="/" className="inline-block mb-5">
              <Image
                src="/logos/logonegrodesktop.png"
                alt="Iron Project"
                width={120}
                height={36}
                className="logo-light-desktop h-7 w-auto opacity-80 hover:opacity-100 transition-opacity"
              />
              <Image
                src="/logos/logoblancodesktop.png"
                alt="Iron Project"
                width={120}
                height={36}
                className="logo-dark-desktop h-7 w-auto opacity-80 hover:opacity-100 transition-opacity"
              />
            </Link>
            <p className="text-sm leading-relaxed mb-6" style={{ color: 'var(--ip-text-muted)' }}>
              Estructuras y muebles de hierro fabricados con precisión artesanal.
            </p>
            <a
              href="https://instagram.com/ironproject"
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs uppercase tracking-widest transition-opacity hover:opacity-60"
              style={{ color: 'var(--ip-text-muted)' }}
            >
              @ironproject
            </a>
          </div>

          {/* Catálogo */}
          <div>
            <h3
              className="text-xs uppercase tracking-widest mb-5 font-medium"
              style={{ color: 'var(--ip-text)' }}
            >
              Catálogo
            </h3>
            <ul className="space-y-3">
              {[
                { label: 'Todo el catálogo', href: '/catalogo' },
                { label: 'Hogar',            href: '/catalogo/hogar' },
                { label: 'Eventos',          href: '/catalogo/eventos' },
                { label: 'Comercial',        href: '/catalogo/comercial' },
                { label: 'A Medida',         href: '/a-medida' },
              ].map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm transition-opacity hover:opacity-60"
                    style={{ color: 'var(--ip-text-muted)' }}
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Empresa */}
          <div>
            <h3
              className="text-xs uppercase tracking-widest mb-5 font-medium"
              style={{ color: 'var(--ip-text)' }}
            >
              Empresa
            </h3>
            <ul className="space-y-3">
              {[
                { label: 'Nosotros',      href: '/nosotros' },
                { label: 'Proyectos',     href: '/proyectos' },
                { label: 'Para Negocios', href: '/para-negocios' },
                { label: 'FAQ',           href: '/faq' },
              ].map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm transition-opacity hover:opacity-60"
                    style={{ color: 'var(--ip-text-muted)' }}
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contacto */}
          <div>
            <h3
              className="text-xs uppercase tracking-widest mb-5 font-medium"
              style={{ color: 'var(--ip-text)' }}
            >
              Contacto
            </h3>
            <p className="text-sm mb-5 leading-relaxed" style={{ color: 'var(--ip-text-muted)' }}>
              Respondemos en menos de 1 hora en horario comercial.
            </p>
            <a
              href={waURL}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 text-sm font-medium px-4 py-2.5 transition-opacity hover:opacity-80"
              style={{ backgroundColor: 'var(--ip-wa)', color: 'var(--ip-wa-text)' }}
            >
              <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor">
                <path d={WA_PATH} />
              </svg>
              Escribinos
            </a>
          </div>
        </div>

        {/* Bottom bar */}
        <div className="flex flex-col sm:flex-row justify-between items-center gap-3 py-5">
          <p className="text-xs" style={{ color: 'var(--ip-text-muted)' }}>
            © {new Date().getFullYear()} Iron Project. Todos los derechos reservados.
          </p>
          <p className="text-xs" style={{ color: 'var(--ip-text-muted)' }}>
            Fabricación artesanal de hierro · Buenos Aires
          </p>
        </div>

      </div>
    </footer>
  )
}
